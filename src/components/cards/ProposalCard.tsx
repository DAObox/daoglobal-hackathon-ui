...
type Props = TokenVotingProposalListItem & { path: "ops" | "lens-voting" };

export const ProposalCard: React.FC<Props> = ({ status, path, ...proposal }) => {
  ...
  return (
    <div className="border-neutral card col-span-1 w-full scale-95 rounded-lg bg-secondary shadow-xl transition hover:scale-100">
      <div
        ...
        onClick={() => router.push(`/${path}/${proposal.id}`)}
      >
          ...
          <div className="flex flex-1 items-center justify-start gap-2 text-sm font-medium">
            <div className="placeholder avatar">
              <div className="w-8 rounded-full bg-success text-neutral-content">
                <span className="text-xs">{upperCase(proposal.dao.name.substring(0, 2))}</span>
              </div>
            </div>
            <p className="text-md">
              {proposal.dao.name} by <strong>{truncateAddress(proposal.dao.address, 8, 6)}</strong>
            </p>
          </div>
