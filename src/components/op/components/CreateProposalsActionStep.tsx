...
import { Action, type CreateProposalDetail, type CreateProposalVoting } from "types";
...
export const CreateProposalsActionStep: React.FC<Props> = ({ onComplete, metadata, onCancel }) => {
  const [actions, setActions] = useState<
    (TransferEncoderProps & { id: number } & { selected?: (typeof availableTokens)[0] })[]
  >([emptyAction(1)]);
  const [proposalSubmitted, setProposalSubmitted] = useState(false);
  const { data, status, write, error, prepareStatus } = useNewOpProposal({
    metadata,
    actions: transferEncoder(
      actions.map(({ to, amount, selected }) => ({
        to: to as Address,
        amount,
        token: (selected?.address ?? "") as Address,
      }))
    ) as unknown as Action[],
    funds: BigNumber.from(10),
    allowFailureMap: BigNumber.from(0),
  });
  ...
  const addAction = () => {
    ...
    setActions((prev) => [...prev, emptyAction(actions_length + 1)]);
  };
    ...
    <div className="flex w-full flex-col gap-10">
      <div>
        ...
        <div className="border-neutral mt-4 flex w-full flex-col items-center justify-center gap-5 rounded-lg border-2 p-8">
          ...
          <p className="text max-w-md text-center">
          ...
          <Dialog.Panel className="m relative flex max-h-full w-full max-w-2xl flex-col items-center justify-center overflow-auto rounded-lg bg-secondary p-10">
            <div>
              <CheckIcon
                width={40}
                height={40}
                stroke={prepareStatus === "error" ? "red" : "green"}
              />
            </div>
